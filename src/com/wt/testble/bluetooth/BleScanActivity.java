...
import com.wt.testble.bluetooth.adapter.ViewHolder;
    ...
    private void initAdapter() {
        ...
        deviceAdapter = new CommonAdapter<Map<String, Object>>(
            ...
            public void convert(ViewHolder holder, final Map<String, Object> deviceMap) {
                holder.setText(R.id.txtv_name, deviceMap.get(SingletonBLE.FIELD_NAME).toString());
                holder.setText(R.id.txtv_address, deviceMap.get(SingletonBLE.FIELD_ADDRESS).toString());
                holder.setText(R.id.txtv_connState, ((boolean) deviceMap.get(SingletonBLE.FIELD_ISCONNECT)) ?
                        getResources().getString(R.string.state_connected) :
                        getResources().getString(R.string.state_disconnected));
                holder.setText(R.id.btn_connect, ((boolean) deviceMap.get(SingletonBLE.FIELD_ISCONNECT)) ?
                ...
                holder.getView(R.id.btn_connect).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if ((boolean) deviceMap.get(SingletonBLE.FIELD_ISCONNECT)) {
                            mBleService.disconnect();
                            showDialog(getString(R.string.disconnecting));
                        } else {
                            connDeviceAddress = (String) deviceMap.get(SingletonBLE.FIELD_ADDRESS);
                            connDeviceName = (String) deviceMap.get(SingletonBLE.FIELD_NAME);
                            HashMap<String, Object> connDevMap = new HashMap<String, Object>();
                            connDevMap.put(SingletonBLE.FIELD_NAME, connDeviceName);
                            connDevMap.put(SingletonBLE.FIELD_ADDRESS, connDeviceAddress);
                            connDevMap.put(SingletonBLE.FIELD_ISCONNECT, false);
                            deviceList.clear();
    ...
    private BroadcastReceiver bleReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(BleService.ACTION_BLUETOOTH_DEVICE)) {
                String tmpDevName = intent.getStringExtra(SingletonBLE.FIELD_NAME);
                String tmpDevAddress = intent.getStringExtra(SingletonBLE.FIELD_ADDRESS);
                Log.i(TAG, "name: " + tmpDevName + ", address: " + tmpDevAddress);
                HashMap<String, Object> deviceMap = new HashMap<>();
                deviceMap.put(SingletonBLE.FIELD_NAME, tmpDevName);
                deviceMap.put(SingletonBLE.FIELD_ADDRESS, tmpDevAddress);
                deviceMap.put(SingletonBLE.FIELD_ISCONNECT, false);
                deviceList.add(deviceMap);
                deviceAdapter.notifyDataSetChanged();
            } else if (intent.getAction().equals(BleService.ACTION_GATT_CONNECTED)) {
                deviceList.get(0).put(SingletonBLE.FIELD_ISCONNECT, true);
                deviceAdapter.notifyDataSetChanged();
                dismissDialog();
            } else if (intent.getAction().equals(BleService.ACTION_GATT_DISCONNECTED)) {
                deviceList.get(0).put(SingletonBLE.FIELD_ISCONNECT, false);
                serviceList.clear();
