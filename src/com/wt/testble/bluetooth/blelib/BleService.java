...
public class BleService extends Service implements Constants, BleListener {
    ...
    public static final long TIMEOUT_SCAN_PERIOD = 10 * 1000;
...
//            mBluetoothAdapter.getBluetoothLeScanner().stopScan(mLeScanCallback);
    ...
    public void scanLeDevice(boolean enable) {
        this.scanLeDevice(enable, TIMEOUT_SCAN_PERIOD);
    }
    ...
    public boolean connect(final String address) {
        if (isScanning) scanLeDevice(false,TIMEOUT_SCAN_PERIOD);
        close();
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
        	...
        	String address = gatt.getDevice().getAddress();
            for (int i = 0; i < characteristic.getValue().length; i++) {
                Log.i(TAG, "address: " + address + ",Read: " + characteristic.getValue()[i]);
            }
            if (mOnDataAvailableListener != null) {
    ...
    private void broadcastUpdate(final String action, final String address) {
        final Intent intent = new Intent(action);
        intent.putExtra(SingletonBLE.FIELD_ADDRESS, address);
        sendBroadcast(intent);
    }

    private void broadcastUpdate(final String action, BluetoothDevice device) {
        final Intent intent = new Intent(action);
        intent.putExtra(SingletonBLE.FIELD_NAME, device.getName());
        intent.putExtra(SingletonBLE.FIELD_ADDRESS, device.getAddress());
        sendBroadcast(intent);
    }
